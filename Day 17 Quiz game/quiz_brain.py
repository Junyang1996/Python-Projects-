class QuizBrain:
    def __init__(self, question_list):
        self.question_number = 0
        self.list = question_list
        self.score = 0

    def next_question(self):
        current_question = self.list[self.question_number]
        current_question_question = current_question.text
        self.question_number += 1
        """retrieve the current question from the current question number and ask the question to the user"""
        answer = input(f"Q. {self.question_number}: {current_question_question} (True/False)?: ")
        self.check_answer(answer , current_question.answer)

    def still_have_questions(self):
        return self.question_number != len(self.list)

    def check_answer(self, user_answer, correct_answer):
        """if the user input is the same as the actual answer"""
        if user_answer.lower() == correct_answer.lower():
            print("You got it right!")
            self.score += 1
        else:
            print("That's wrong.")
        print(f"The correct answer was: {correct_answer}.")
        print(f"Your current score is: {self.score}/{self.question_number}")